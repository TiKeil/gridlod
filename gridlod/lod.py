...
class SeparatedCoarseScaleInformation:
    def __init__(self, Kij, Kmsij, Qmsij):
        self.Kij = Kij
        self.Kmsij = Kmsij
        self.Qmsij = Qmsij
...
def computeSeparatedBasisCoarseQuantities(patch, correctorsList, aPatch):
    ''' Compute the coarse quantities for the basis and its correctors
    ...
    Kmsij = np.zeros((NpPatchCoarse, numLambdas))
    Qmsij = np.zeros((NpPatchCoarse, numLambdas))
    Kij = np.zeros((numLambdas, numLambdas))
